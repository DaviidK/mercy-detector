...
void classifier_detection::cascadeClassifierSetup(const vector<OWConst::Heroes>& heroesToDetect) {
	for (int i = 0; i < heroesToDetect.size(); i++) {
        ...
        string classifierFilePath = this->classifierDirectory + heroesToDetect[i].getHeroStirng() + ".xml";

        if (!heroClassifier.load(classifierFilePath)) {
            //throw invalid_argument("No available classifier for given hero");
            cout << "ERROR!: Cannot load classifier for " << heroesToDetect[i].getHeroString() << endl;
            break;
        }
        else {
...
*/
void classifier_detection::identifyHero(vector<CascadeClassifier> classifier, Mat image, vector<OWConst::Heroes> knownHero) {
    Mat frame_gray;
